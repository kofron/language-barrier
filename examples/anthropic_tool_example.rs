    ...
    fn execute(&self, name: &str, arguments: Value) -> Result<String> {
        match name {
            ...
            "calculate" => {
                ...
                // For this example, let's assume the expression is valid and return a mock result
                let result = match request.expression.as_str() {
                    "2+2" => "4",
                    "3*4" => "12",
    ...
    fn execute_typed(&self, request: ExampleToolRequest) -> Result<String> {
        match request {
            ...
            ExampleToolRequest::Calculator(calc_req) => {
                // This is a simple example - in a real implementation, you'd evaluate the expression
                let result = match calc_req.expression.as_str() {
                    "2+2" => "4",
                    "3*4" => "12",
                    _ => "Result would be calculated here",
                };
...
fn main() -> Result<()> {
    // For a real example, you would need an API key
    // This example is structured to show the conversions, not to make real API calls
    
    // Create a provider - in a real application, use your API key
    // We don't actually use this in our example, since we're not making API calls
    ...
    println!("Chat created with toolbox containing these tools:");
    ...
    println!("Request creation skipped for this example");
    
    // Demonstrating tool call handling
    ...
    println!("\nUsing typed view for tool calls:");
    ...
    println!("\nThis example demonstrated how the Anthropic provider handles tool calls:");
    println!("1. Tool descriptions are automatically converted to Anthropic's format in the request");
    println!("2. Tool calls in the assistant's message are parsed and executed");
    println!("3. Results from tools are converted to Anthropic's tool_result format");
    println!("4. The typed view pattern provides a type-safe interface for working with tool calls");
