...
fn main() -> Result<()> {
    ...
    println!(
        "1. Tool descriptions are automatically converted to Anthropic's format in the request"
    );
    ...
    println!(
        "4. The typed view pattern provides a type-safe interface for working with tool calls"
    );
